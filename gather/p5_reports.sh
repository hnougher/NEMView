...
logfile=/usr/local/lsws/social-japan.bnr.la/gather/p5_reports.log
if [ -f $logfile ]; then
...
previous=$(psql -h 10.240.0.165 -U nem_worker -d nem -t -c "SET TIME ZONE '+10';SELECT to_char(MAX(run_datetime),'YYYYMMDDHH24MI') FROM p5_regionsolution;")
previous=${previous#*$'\n'}
echo "Previous: $previous"

# Get list of filenames from the AEMO website, and remove any that the middle number is less than or equal to $previous
rm $working_dir/filenames_p5_reports.html -f
wget -O $working_dir/filenames_p5_reports.html http://nemweb.com.au/Reports/CURRENT/P5_Reports/
filenames=$(grep -o "/PUBLIC_P5MIN_[0-9]*_[0-9]*" $working_dir/filenames_p5_reports.html | sed 's#^/##' | awk -v previous="$previous" -F_ '$3 > previous {print}')
rm $working_dir/filenames_p5_reports.html -f
...
rm $working_dir/PUBLIC_P5MIN_* -f
...
do
	...
	COUNT=10
	while [ $COUNT -gt 0 ]; do
		if wget -O $working_dir/$filename.zip http://nemweb.com.au/Reports/CURRENT/P5_Reports/$filename.zip; then
			break
		fi
		COUNT=$((COUNT - 1))
		sleep 1
	...
	# Remove everything before a line starting with "I,P5MIN,REGIONSOLUTION,", keeping the line
	# And remove everything from a line starting with "I," onwards
	awk '/^I,P5MIN,REGIONSOLUTION,/{flag=1;print;next}/^I,/{flag=0}flag' $working_dir/$filename.CSV > $working_dir/$filename.processed.CSV

	# Process the file part 2
	# Remove the first 4 columns of the CSV
	mlr --csv cut -x -f "I,P5MIN,REGIONSOLUTION,10" $working_dir/$filename.processed.CSV > $working_dir/$filename.final.CSV
...
SET TIME ZONE '+10';
START TRANSACTION;
TRUNCATE p5_regionsolution;
\copy p5_regionsolution FROM '$working_dir/$filename.final.CSV' DELIMITER ',' CSV HEADER
COMMIT;
EOF
