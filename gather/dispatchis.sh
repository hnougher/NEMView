...
logfile=/usr/local/lsws/social-japan.bnr.la/gather/dispatchis.log
if [ -f $logfile ]; then
...
do
	...
	COUNT=10
	while [ $COUNT -gt 0 ]; do
		if wget -O $working_dir/$filename.zip http://nemweb.com.au/Reports/CURRENT/DispatchIS_Reports/$filename.zip; then
			break
		fi
		COUNT=$((COUNT - 1))
		sleep 1
